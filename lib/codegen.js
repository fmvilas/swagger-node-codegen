...
const generateOperationFile = (config, operation, operation_name) => new Promise((resolve, reject) => {
  // CJD:  We only want to generate .controller and .interface files using this method
  if (config.file_name.indexOf('.controller.') < 0 && config.file_name.indexOf('.interface.') < 0) {
    resolve();
    return;
  }
...
const generateSchemaFile = (config, model, modelName) => new Promise((resolve, reject) => {
  // CJD:  We only want to generate .model files using this method
  if (config.file_name.indexOf('.model.') <= 0) {
    resolve();
    return;
  }
  ...
  fs.readFile(path.join(config.root, config.file_name), 'utf8', (err, data) => {
    if (err) return reject(err);
    const subdir = config.root.replace(new RegExp(`${config.templates_dir}[/]?`),'');
    const new_filename = config.file_name.replace('___', modelName);
    const target_file = path.resolve(config.target_dir, subdir, new_filename);
    const template = Handlebars.compile(data.toString());
 ...
 */
 const generateSchemaFiles = config => new Promise((resolve, reject) => {
  const files = {};
  _.each(config.data.swagger.components.schemas, (schema, modelName) => {
    files[modelName] = schema;

    Promise.all(
      _.map(files, (schema, modelName) => generateSchemaFile(config, schema, modelName))
    ...
    walker.on('file', async (root, stats, next) => {
      try {
        if (stats.name.substr(0,3) === '___') {
          ...
          // these file should be handled for each in swagger.paths
