...
const generateServices = config => new Promise((resolve, reject) => {
  ...
  _.each(config.swagger.paths, (path, path_name) => {
    const service_name = path_name === '/' ? 'root' : path_name.split('/')[1];
  ...
  _.each(services, async (service, service_name) => {
    try {
      await generateService(config, service, service_name);
    } catch (e) {
      reject(e);
    }
  });
...
codegen.generate = config => new Promise((resolve, reject) => {
  ...
  try {
    config.swagger = swagger2(config.swagger);
  } catch (e) {
    return reject(e);
  }
